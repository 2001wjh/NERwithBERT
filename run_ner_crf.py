    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if "f1" in results and results['f1']>best_f1:
                    if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                        best_f1 = results['f1']
                        logger.info(f"Best Score is {best_f1}")
                        save_model(args,model,tokenizer,scheduler)
...
# 功能：模型保存
def save_model(args,model,tokenizer,scheduler):
...
def predict(args, model, tokenizer, prefix=""):
    ...
    elif args.task_name in ['CMeEE']:
        output_submit_file = os.path.join(pred_output_dir, prefix, f"{args.task_name}_test_submit.json")
        test_dic_lis = load_json(os.path.join(args.data_dir,f"{args.task_name}_test.json"))
        test_pred_dic_lis = []
        for pred_dic,test_dic in zip(results,test_dic_lis):
            text = test_dic["text"]
            entity_lis = []
            for entity in pred_dic['entities']:
                entity_lis.append({
                    "entity":text[entity[1]:entity[2]],
                    "start_idx": entity[1],
                    "end_idx": entity[2],
                    "type": entity[0]
                })
            test_pred_dic_lis.append({
                "text":text,
                "entities":entity_lis
            })
        write_json(output_submit_file,test_pred_dic_lis)
...
def main():
    ...
    if not os.path.exists(args.output_dir):
        os.mkdir(args.output_dir)
    args.output_dir = os.path.join(args.output_dir,"crf") 
    if not os.path.exists(args.output_dir):
        os.mkdir(args.output_dir)
