...
import copy
import json
from .utils_ner import DataProcessor,load_json
...
def convert_examples_to_features(examples,label_list,max_seq_length,tokenizer,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if len(label_ids) != max_seq_length:
            ...
            print(f"len(label_ids):{len(label_ids)}")
...
class MsraProcessor(DataProcessor):
    ...
    def get_labels(self,data_dir=""):
        """See base class."""
        return [
            "X", "B-ORG", "I-ORG", 'B-LOC', 'I-LOC', 
            'B-PER', 'I-PER','O',"[START]", "[END]"
        ]
...
class CMeEEProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_json(os.path.join(data_dir, "CMeEE_train.json")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_json(os.path.join(data_dir, "CMeEE_dev.json")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_json(os.path.join(data_dir, "CMeEE_test.json")), "test")

    def get_labels(self,data_dir=""):
        """See base class."""
        labels_set = set()
        data_list = load_json(os.path.join(data_dir, "CMeEE_train.json"))
        for data in data_list:
            entities = data.get("entities",None)
            if entities is not None:
                for entity in entities:
                    labels_set.add(entity['type'])
        
        return ["X"]+[f"{tag}-{label}" for label in labels_set for tag in ["B","I","S"]]+['O',"[START]", "[END]"]
    ...
    def _read_json(self,input_file):
        ...
        for index,data in enumerate(data_list):
            ...
            if entities is not None:
                for entity in entities:
                    if ''.join(words[entity['start_idx']:entity['end_idx']+1]) == entity['entity']:
                        if entity['start_idx'] == entity['end_idx']:
                            labels[entity['start_idx']] = 'S-'+entity['type']
                        else:
                            labels[entity['start_idx']] = 'B-'+entity['type']
                            labels[entity['start_idx']+1:entity['end_idx']+1] = ['I-'+entity['type']]*(len(entity['entity'])-1)
                    else:
                        logger.info(f"fun _read_json => data:{data}")
            word_label = {"words": words, "labels": labels}
            if len(words)==len(labels): 
                lines.append(word_label)
            else:
                logger.info(f"index:{index}  word_label:{word_label}")
        return lines
