...
import copy
import json
from .utils_ner import DataProcessor,get_entities,load_json,write_json
...
class CMeEEProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            labels = line['labels']
            if i<10:
                logger.info("Writing line %d of %s", i, str(line))
            ...
            if entities is not None:
                for entity in entities:
                    if ''.join(words[entity['start_idx']:entity['end_idx']+1]) == entity['entity']:
                        labels.append([
                            entity['type'],entity['start_idx'],entity['end_idx']
                        ])
                    else:
                        logger.info(f"fun _read_json => index:{index} => {''.join(words[entity['start_idx']:entity['end_idx']+1])} != {entity['entity']}")
